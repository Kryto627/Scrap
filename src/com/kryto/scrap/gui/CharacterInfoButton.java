...
public class CharacterInfoButton extends Component {
	...
	public void render() {
		...
		if (isMouseOver()) {
			...
			Assets.PIXEL_OPERATOR_SMALL.renderCentered("HP: " + character.getStats().getMaxHealth(), (float)bounds.getCenterX(), (float)(bounds.getY() + bounds.getHeight() + 55), Color.green);
			Assets.PIXEL_OPERATOR_SMALL.renderCentered("ATTACK: " + character.getStats().getMaxDamage(), (float)bounds.getCenterX(), (float)(bounds.getY() + bounds.getHeight() + 80), Color.red);		
